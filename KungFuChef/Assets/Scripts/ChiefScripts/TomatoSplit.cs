...
public class TomatoSplit : MonoBehaviour
{
    ...
    void split()
    {
        ...
        left.GetComponent<Rigidbody>().AddForce(-wrap.transform.right * 0.5f, ForceMode.Impulse);
        right.GetComponent<Rigidbody>().AddForce(wrap.transform.right * 0.5f, ForceMode.Impulse);
        left.transform.parent = transform.parent.parent;
        right.transform.parent = transform.parent.parent;
        middle.transform.parent = transform.parent.parent;
        GetComponent<MeshRenderer>().enabled = false;
        tomatoUncut.GetComponent<MeshRenderer>().enabled = false;
        Destroy(wrap, 5f);
